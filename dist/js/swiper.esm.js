...
const Scale = {
  setTranslate() {
    ...
    const params = swiper.params.scaleEffect;
    ...
    for (let i = 0, length = slides.length; i < length; i += 1) {
      ...
      let translateZ = Math.abs(offsetMultiplier) >= 0 && Math.abs(offsetMultiplier) < 1 ? -translate * Math.abs(offsetMultiplier) : -translate;
...
var effectScale = {
  ...
  create() {
    ...
    Utils.extend(swiper, {
      scaleEffect: {
        setTranslate: Scale.setTranslate.bind(swiper),
        setTransition: Scale.setTransition.bind(swiper),
      },
    });
  },
  on: {
    beforeInit() {
      const swiper = this;
      if (swiper.params.effect !== 'scale') return;

      swiper.classNames.push(`${swiper.params.containerModifierClass}scale`);
    ...
    setTranslate() {
      const swiper = this;
      if (swiper.params.effect !== 'scale') return;
      swiper.scaleEffect.setTranslate();
    },
    setTransition(duration) {
      const swiper = this;
      if (swiper.params.effect !== 'scale') return;
      swiper.scaleEffect.setTransition(duration);
    },
  },
};
...
export { Swiper, virtual as Virtual, keyboard as Keyboard, mousewheel as Mousewheel, navigation as Navigation, controller as Controller, a11y$1 as A11y, hashNavigation as HashNavigation, effectScale as EffectScale };
